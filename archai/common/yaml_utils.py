...
def _join_path(path1:str, path2:str):
    mid = (1 if path1.endswith('/') else 0) + (1 if path2.startswith('/') else 0)
...
def _rel2full_path(cwd:str, rel_path:str) -> str:
    ...
    """
    assert cwd != "" and cwd.startswith('/'), 'cwd must be absolute path'

    rel_parts = rel_path.split('/')
    cwd_parts = [] if rel_path.startswith('/') else cwd.split('/')
    ...
    assert '..' not in final and is_proper_path(final)
    return final
