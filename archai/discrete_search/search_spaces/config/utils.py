from typing import Dict, Callable, Type, Iterable, Union, Any
...
def replace_ptree_choices(config_tree: Union[Dict, DiscreteChoice],
                          repl_fn: Callable[[DiscreteChoice], Any]):
...
def replace_ptree_pair_choices(query_tree: Union[Dict, DiscreteChoice],
                                    aux_tree: Union[Dict, Any],
                                    repl_fn: Callable[[DiscreteChoice, Any], Any]):
    ...
    def _replace_tree_nodes(query_node, aux_node, repl_fn, ref_map):
        if isinstance(query_node, dict):
            ...
            for param_name, param in query_node.items():
                assert param_name in aux_node,\
                    '`aux_tree` must be identical to `query_tree` apart from terminal nodes'

                output_tree[param_name] = _replace_tree_nodes(
                    param, aux_node[param_name], repl_fn, ref_map
                )
